...
public class ResetPwdModel extends BaseModel {
    ...
    public void resetpwd(String mobile,String capcha,String password ,OnPresenterListener.OnModelListener<BaseBean<CheckSmsCodeBean>> listener){
        mSubscription = UserAPI.resetPassword(mobile, capcha, password, new Observer<BaseBean<CheckSmsCodeBean>>() {
            @Override
            ...
            @Override
            public void onNext(BaseBean<CheckSmsCodeBean> bean) {
                new Handler().post(() -> {
                    Log.i(TAG, "onNext run: "+bean.toString());
                    if (listener != null) {
